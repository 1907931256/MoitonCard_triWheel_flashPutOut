...
int ReadFlashPointInformation(long long address)
{
	...
	//读取点个数
	STMFLASH_Read(address,&numCount,1);

	//读取点信息到ringBuffer
	STMFLASH_Read(address+4,GetFristAdress(),numCount*7);
...
int PoseSamplingDone(long long address)
{
	...
	SpeedPlaning();
	//将点个数写在flash
	STMFLASH_Write(address,&numCount,1);
	//将点信息写在flash
	STMFLASH_Write(address+4,GetFristAdress(),numCount*7);
